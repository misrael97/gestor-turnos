...
export class ColasPage implements OnInit {
  ...
  async cargarTurnos(event?: any) {
    ...
    this.http.get(`${environment.apiUrl}/turnos`, { headers: this.auth.headers }).subscribe({
      next: (res: any) => {
        // ✅ Usar estados correctos: 'espera' y 'llamado' (no 'en espera')
        this.turnos = res.filter((t: any) => t.estado === 'espera' || t.estado === 'llamado');
        this.loading = false;
  ...
  async ejecutarReasignacion(turnoId: number, nuevaCola: string) {
    ...
    // ✅ Usar el endpoint correcto: /turnos/{id}/reasignar-cola
    this.http.put(`${environment.apiUrl}/turnos/${turnoId}/reasignar-cola`, 
      { cola: nuevaCola }, 
